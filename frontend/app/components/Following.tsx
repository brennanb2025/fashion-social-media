...
import styles from '../styles/followersFollowing.module.css'
import Link from 'next/link';
import UserProfilePicture from './UserProfilePicture';
import followUser from '../utils/users/follow';
...
const UserFollowingList: React.FC<UserFollowingListProps> = ({ user, loggedInUser }) => {
  ...
  const fetchUserFollowing = () => {
    ...
    axios.get<User[]>(`http://localhost:8000/api/users/${user.id}/following/`)
      .then((res) => setUserFollowingList(res.data))
  ...
  const renderUserList = () => {
    return userFollowingList.map((following) => (
      <div key={following.id}>
        <div className='row'>
            ...
            <div className={`${styles.colCenterButton} col`}>
                <button 
                        onClick={() => {followAnotherUser(following)}}
